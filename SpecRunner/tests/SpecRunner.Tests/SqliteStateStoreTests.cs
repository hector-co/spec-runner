...
public class SqliteStateStoreTests : IDisposable
{
    ...
    public async Task FindByCommentIdReturnsOwningTrackedIssue()
    {
        ...
        await store.UpsertCommentAsync(45, comment);
    ...
    public async Task UpsertingExistingTrackedCommentUpdatesStatus()
    {
        var store = new SqliteStateStore(_filePath);
        await store.UpsertTrackedIssueAsync(new TrackedIssue(45, "45-add-login-page"));
        await store.UpsertCommentAsync(45, new TrackedComment(9001, CommentKind.IssueComment, CommentStatus.Pending));

        await store.UpsertCommentAsync(45, new TrackedComment(9001, CommentKind.IssueComment, CommentStatus.Done));
    ...
    public async Task CommentKindIsPersistedAndReadBackCorrectly()
    {
        var store = new SqliteStateStore(_filePath);
        await store.UpsertTrackedIssueAsync(new TrackedIssue(45, "45-add-login-page"));
        await store.UpsertCommentAsync(45, new TrackedComment(1, CommentKind.IssueComment, CommentStatus.Pending));
        await store.UpsertCommentAsync(45, new TrackedComment(2, CommentKind.PrIssueComment, CommentStatus.Pending));
        await store.UpsertCommentAsync(45, new TrackedComment(3, CommentKind.PrReviewComment, CommentStatus.Pending));
