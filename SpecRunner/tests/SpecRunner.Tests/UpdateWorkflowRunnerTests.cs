...
public class UpdateWorkflowRunnerTests : IDisposable
{
    ...
    private (UpdateWorkflowRunner Runner, RecordingGitService Git, RecordingGitHubService GitHub, FakeCliAgentSessionFactory CliFactory, SqliteStateStore StateStore) CreateRunner(
    ...
    {
        var (runner, _, gitHub, _, _) = CreateRunner();
    ...
    {
        var (runner, git, gitHub, cliFactory, _) = CreateRunner();
        gitHub.PullRequests.Add(PullRequest(12, "feature/45"));
    ...
    {
        var (runner, git, gitHub, cliFactory, stateStore) = CreateRunner();
        gitHub.PullRequests.Add(PullRequest(12, "feature/45"));
        gitHub.PrComments[12] = new List<PrComment> { Comment(9001, "/update") };
    ...
    {
        var (runner, git, gitHub, _, stateStore) = CreateRunner();
