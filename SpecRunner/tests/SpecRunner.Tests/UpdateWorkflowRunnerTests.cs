...
public class UpdateWorkflowRunnerTests : IDisposable
{
    private readonly string _tempDirectory;
    private readonly string _stateFilePath;

    public UpdateWorkflowRunnerTests()
    {
    ...
    private (UpdateWorkflowRunner Runner, RecordingGitService Git, RecordingGitHubService GitHub, FakeCliAgentSessionFactory CliFactory, SqliteStateStore StateStore, FakeTasksFileReader TasksFileReader, FakePrAdoptionService PrAdoptionService) CreateRunner(
        ...
        Func<ICliAgentSession>? sessionFactory = null)
    {
        ...
        var runner = new UpdateWorkflowRunner(gitHub, git, stateStore, prAdoptionService, cliFactory, tasksFileReader, commandTemplateRenderer, options, NullLogger<UpdateWorkflowRunner>.Instance);
    ...
    [InlineData("/update")]
    [InlineData("/update\nAdditional context")]
    [InlineData("/update please handle this")]
    [InlineData("  /update  ")]
    ...
    [Theory]
    [InlineData("/updated")]
    [InlineData("please /update this")]
    [InlineData("/updatework")]
    [InlineData("some text mentioning /update mid-sentence")]
    ...
    public async Task InstructionsStripLeadingTriggerTokenAndSeparatingWhitespace()
    {
        ...
        gitHub.PullRequests.Add(PullRequest(12, "feature/45"));
        gitHub.PrComments[12] = new List<PrComment> { Comment(9001, "/update add validation for the email field") };
    ...
    public async Task UnauthorizedAuthorCommentIsIgnored()
    {
        ...
        gitHub.PullRequests.Add(PullRequest(12, "feature/45"));
        gitHub.PrComments[12] = new List<PrComment> { Comment(9001, "/update", author: "rando", authorAssociation: "NONE") };
    ...
    public async Task CommentWithExistingBotReactionIsSkipped()
    {
        var (runner, git, gitHub, cliFactory, _, _, _) = CreateRunner();
        gitHub.PullRequests.Add(PullRequest(12, "feature/45"));
        gitHub.PrComments[12] = new List<PrComment> { Comment(9001, "/update") };
    ...
    public async Task SuccessfulAdoptionWithIssueProceedsLikeATrackedPr()
    {
        ...
        gitHub.PullRequests.Add(PullRequest(12, "contributor/csv-export"));
        gitHub.PrComments[12] = new List<PrComment> { Comment(9001, "/update") };
        ...
        Assert.Contains("Commit:updating specs for #45", git.Calls);
    ...
    public async Task SuccessfulAdoptionWithoutIssueProceedsWithoutOne()
    {
        ...
        Assert.Contains("Commit:updating specs for PR #12", git.Calls);
        Assert.Contains((9001L, "+1"), gitHub.AddedReactions);
    ...
    public async Task SuccessfulRunRefreshesBranchCommitsPushesAndUpdatesState()
    {
        ...
        gitHub.PullRequests.Add(PullRequest(12, "feature/45"));
        gitHub.PrComments[12] = new List<PrComment> { Comment(9001, "/update the export button must also support CSV") };

        await runner.RunOnceAsync();

        Assert.Equal(
            new[] { "ResetHard:HEAD", "Fetch:feature/45", "SwitchBranch:feature/45", "ResetHard:origin/feature/45", "Commit:updating specs for #45", "Push:feature/45" },
            git.Calls);
    ...
    public async Task TrackedBranchNameIsUsedEvenWhenItDiffersFromPrHeadBranch()
    {
        ...
        Assert.Equal(
            new[] { "ResetHard:HEAD", "Fetch:feature/45-2", "SwitchBranch:feature/45-2", "ResetHard:origin/feature/45-2", "Commit:updating specs for #45", "Push:feature/45-2" },
            git.Calls);
    }
    ...
    public async Task ThrownExceptionDuringProcessingIsReportedAndProcessingContinuesToNextComment()
    {
        ...
        gitHub.PrComments[13] = new List<PrComment> { Comment(9002, "/update") };
