...
TBD - defines `ISpecFolderResolver`, which reconciles an expected
spec/change name (from `ISpecNameResolver`) with the actual folder that
the CLI coding agent created on disk under `openspec/changes/`, falling
back to a prefix match keyed by issue number and reporting an error when no
matching folder exists at all.
