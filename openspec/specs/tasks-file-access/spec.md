...
TBD - defines the `ITasksFileReader` capability: reading a change's current
`tasks.md` content (pre-archive) and its archived `tasks.md` content
(post-`openspec archive`) from the local working tree, given a resolved
spec/change name, so the propose, implement, and finalize workflows can
source PR descriptions from it.
