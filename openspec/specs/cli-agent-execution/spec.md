...
messages, newline-delimited JSON on stdout for agent events), so that
prompts, follow-up commands, and interrupts are exchanged over the running
process's stdin/stdout rather than by restarting the process per turn.
