...
TBD - defines adoption of a PR that has no `IStateStore` record: discovering
its spec/change folder and, optionally, an associated issue, purely by
reading from GitHub and the local git clone, and upserting a new tracked
record so `implement-workflow`, `update-workflow`, and `finalize-workflow`
can proceed with it exactly as they would for an already-tracked PR.
