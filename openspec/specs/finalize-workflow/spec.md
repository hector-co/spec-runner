...
tracked record), push the branch via `IGitService.PushAsync`, and then
mark the PR ready for review via `IGitHubService.MarkPrReadyForReviewAsync`.
The workflow SHALL NOT create a new branch or a new pull request, since
the PR already exists.

#### Scenario: Successful session results in a push and a ready-for-review PR
- **WHEN** the CLI agent session for a tracked PR with issue number `45`
  on branch `"feature/45"` and PR number `12` reaches state `Completed`
- **THEN** the changes SHALL be committed with message `"finalizing specs
  for #45"`, the `"feature/45"` branch SHALL be pushed to `origin`, and PR
  `12` SHALL be marked ready for review, with no new branch or pull
  request created
