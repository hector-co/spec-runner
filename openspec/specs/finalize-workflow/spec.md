...
### Requirement: An eligible `/finalize` comment is only processed if its author is authorized
...
#### Scenario: A `/finalize` comment from an unauthorized author is silently skipped
- **WHEN** a scan pass finds a PR comment whose trimmed body is exactly
  `/finalize`, posted by an author whose `author_association` is `"NONE"`
...
#### Scenario: A `/finalize` comment from an authorized author is processed as before
- **WHEN** a scan pass finds a PR comment whose trimmed body is exactly
  `/finalize`, posted by an author whose `author_association` is `"OWNER"`
- **THEN** that comment SHALL be treated as an eligible trigger and
  processed following the existing `/finalize` workflow, unchanged by this
  requirement
