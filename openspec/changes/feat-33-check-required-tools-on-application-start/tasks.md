...
- [x] 2.2 Map a zero exit code to `Available`, a non-zero exit code to
      ...
      failure)
- [x] 2.3 Add unit tests for `ProcessCliToolAvailabilityChecker` covering
...
- [x] 3.1 Implement `StartupDependencyChecker : IStartupDependencyChecker`
...
- [x] 3.2 Ensure a failure in one check does not prevent the remaining
      checks from running
- [x] 3.3 Add unit tests for `StartupDependencyChecker` covering: all
      checks succeed, one check fails but all three still run, and result
      ordering

## 4. Wiring and Program.cs

- [x] 4.1 Register `IOptions<OpenSpecCliOptions>` bound to the
      `OpenSpecCli` configuration section in `Program.cs`
- [x] 4.2 Register `ICliToolAvailabilityChecker` and
      `IStartupDependencyChecker` in the DI container in `Program.cs`
- [x] 4.3 Replace the existing direct `IRepositoryConnectionTester` call in
      `Program.cs` with a call to
      `IStartupDependencyChecker.CheckAllAsync()`
- [x] 4.4 Log each dependency result individually (`LogInformation` on
      success, `LogError` on failure) and print a corresponding line to the
      console for each, without ever including `GitHubToken`
- [x] 4.5 If any result is unsuccessful, log a final error summarizing the
...
- [x] 5.1 Add a default `OpenSpecCli` section (with `Executable`:
      `"openspec"`) to `appsettings.json`
- [x] 5.2 Update `SpecRunner/README.md` (if it documents startup/config
