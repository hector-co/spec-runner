...
- Task 3.4 describes `ReportSuccessAsync`'s upsert as running "unchanged" with the same three logical inputs (issue number, resolved spec name, PR number). Taken literally, that upsert's `TrackedIssue.BranchName` would default to `string.Empty` and — via the task 2.3 widened `ON CONFLICT` clause — erase the branch name recorded moments earlier by the task 3.3 early upsert, contradicting this proposal's own "final report ... leaving its previously recorded branch name intact" requirement. `ReportSuccessAsync` was given an additional `branchName` parameter (the same value already in scope in `ProcessCommentAsync` from branch creation) purely to preserve that value through the second upsert; its three documented logical inputs are otherwise unchanged.
- Task 7.1's `dotnet test` run shows 8 pre-existing failures (`CommandTemplateRendererTests` and prompt-assertion tests in the four `*WorkflowRunnerTests` suites), all differing only by `\n` vs `\r\n` in an embedded template's line endings. Confirmed via a stash-and-rerun against unmodified `HEAD` that these fail identically without any of this change's edits — they stem from this Windows checkout's `core.autocrlf` converting the checked-in LF template file to CRLF, not from anything in this change. Treated as out of scope and left as-is; all tests touched by this change (the new `GitServiceTests`, `SqliteStateStoreTests`, and the new/updated `*WorkflowRunnerTests` branch-sequencing cases) pass.
