...
session. The workflow SHALL deregister the run for that PR number in its
existing `finally` block, after disposing the session.

#### Scenario: A run is registered before the CLI agent starts
- **WHEN** the workflow begins processing an eligible `/implement` comment
  for PR `12`
...
- **WHEN** the workflow finishes processing an eligible `/implement`
  comment for PR `12`, whether by success, error, or cancellation
...
- **THEN** the session SHALL be stopped via `StopAsync`, but no reaction
  ...
  be made by `ImplementWorkflowRunner` for that outcome
