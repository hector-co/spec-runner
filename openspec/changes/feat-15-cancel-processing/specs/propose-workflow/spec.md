...
token source, register an `ActiveRun` for the triggering comment's issue
...
`ProcessCommentAsync` call. Once a CLI-agent session is created for the
comment, the workflow SHALL update the registered `ActiveRun` to expose that
session. The workflow SHALL deregister the run for that issue number in its
existing `finally` block, after disposing the session.

#### Scenario: A run is registered before the CLI agent starts
- **WHEN** the workflow begins processing an eligible `/propose` comment for
  issue `45`
- **THEN** `IActiveRunRegistry` SHALL expose an `ActiveRun` for issue `45`
...
- **WHEN** the workflow finishes processing an eligible `/propose` comment
  for issue `45`, whether by success, error, or cancellation
...
recording the comment's processing status as `error`, whenever any step of
...
- **WHEN** `cancel-workflow` cancels the registered cancellation token
  source for issue `45` while its CLI agent session is running, before
...
- **THEN** the session SHALL be stopped via `StopAsync`, but no reaction
  ...
  be made by `ProposeWorkflowRunner` for that outcome
