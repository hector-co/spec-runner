...
- [x] 1.1 Create `README.md` at the repository root with a title and short
      project summary for SpecRunner.
- [x] 1.2 Add a "Build" section documenting `build/build.ps1`, its
...
- [x] 2.1 Add a "Deploy" section stating the built output must be copied
...
- [x] 2.2 Note that `/.specrunner` is already present in `.gitignore`, so
      the deployed binaries and local `state.db` are not committed.

## 3. Configuration section

- [x] 3.1 Document the `SpecRunner` section of `appsettings.json`:
...
- [x] 3.2 Document the `CliAgent` section (`Executable`, default `claude`)
      and the `OpenSpecCli` section (`Executable`, default `openspec`).
- [x] 3.3 State the prerequisites: an authenticated `claude` CLI and an
