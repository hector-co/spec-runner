...
- [x] 1.1 Create the `SpecRunner.Cli` project under `SpecRunner/src/SpecRunner.Cli`,
...
- [x] 1.2 Register `SpecRunner.Cli` in `SpecRunner.sln`.
- [x] 1.3 Add a `SpecRunner.Cli` project reference to
      `SpecRunner.Console.csproj`.

## 2. Configuration model

- [x] 2.1 Add `CliAgentOptions` to `SpecRunner.Core/Configuration` with
...
- [x] 3.5 Add `ICliAgentSessionFactory` interface to
      `SpecRunner.Core/Abstractions` with
      `ICliAgentSession CreateSession()`.

## 4. Claude CLI session implementation

- [x] 4.1 Add `ClaudeCliAgentSession` to `SpecRunner.Cli` implementing
      ...
      `NotStarted`.
- [x] 4.2 Implement an async stdout reader that parses newline-delimited
...
- [x] 4.3 Implement `SendCommandAsync`: writes a new stream-json user-turn
      message to stdin when `State == Running`; throws
      `InvalidOperationException` otherwise.
- [x] 4.4 Implement `CancelCurrentRequestAsync`: writes a stream-json
...
- [x] 4.5 Implement `StopAsync`: closes stdin, waits briefly for graceful
      exit, force-kills the process if it hasn't exited within the grace
      period, and transitions state to `Stopped`.
- [x] 4.6 Implement `IAsyncDisposable`: calls `StopAsync` if the session
      is not already in a terminal state (`Completed`, `Failed`,
      `Stopped`).
- [x] 4.7 Wire process-exit handling: exit code `0` following a
...
- [x] 4.8 Add `ClaudeCliAgentSessionFactory` implementing
...
- [x] 6.8 Add unit tests for process-exit handling: zero exit with a
      ...
      event.
- [x] 6.9 Add/update the DI smoke test to confirm
      `ICliAgentSessionFactory` resolves from the container.
- [x] 6.10 Verify `dotnet test SpecRunner/SpecRunner.sln` passes.

## 7. Documentation

- [x] 7.1 Update `SpecRunner/README.md` to describe the `CliAgent`
...
- [x] 8.2 Run `dotnet test SpecRunner/SpecRunner.sln` and confirm all
      tests pass.
- [x] 8.3 Manually start a session against a real `claude` executable
