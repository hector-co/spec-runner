...
startup on every invocation, log the resulting status and message, and
print a summary of the connection state to the console. The process SHALL
exit with code `0` when the status is `Connected`, and exit with a non-zero
code for every other status.

#### Scenario: Successful run exits zero
- **WHEN** the console app is run with a valid `RepositoryUrl` and
  `GitHubToken` that resolve to status `Connected`
...
- **THEN** the process SHALL print/log that status and its message and
