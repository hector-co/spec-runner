...
- [ ] 1.1 In `SpecRunner/src/SpecRunner.Console/CommandTemplateRenderer.cs`,
      add a private helper that escapes a single value by first replacing
      `\` with `\\`, then replacing `"` with `\"`.
- [ ] 1.2 Apply that helper to the matched value inside the
...
- [ ] 2.1 In `SpecRunner/tests/SpecRunner.Tests/CommandTemplateRendererTests.cs`,
...
- [ ] 2.2 Add a test with a placeholder value containing a backslash
...
- [ ] 2.3 Add a test confirming a placeholder value with no `"` or `\`
      characters renders unchanged (regression guard against
      over-escaping).

## 3. Verify

- [ ] 3.1 Run `dotnet test` for `SpecRunner.Tests` and confirm all tests,
