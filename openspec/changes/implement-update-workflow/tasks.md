...
- [ ] 1.1 Add `IUpdateWorkflowRunner` to `SpecRunner.Core/Abstractions`
...
- [ ] 1.2 Add a supporting model for an eligible `/update` comment
...
- [ ] 2.2 Implement the eligible-comment scan: list open PRs, read each
...
- [ ] 2.3 Implement sequential per-comment processing: for each eligible
      comment, add the `eyes` reaction first, then look up
      `IStateStore.FindByPrNumberAsync` for that comment's PR.
- [ ] 2.4 Implement the untracked-PR path: post a reply explaining no
...
- [ ] 2.5 Implement the tracked-PR path: `FetchAsync` the PR's head
      branch, `SwitchBranchAsync` to it, `ResetHardAsync` it to
      `origin/{branch}`.
- [ ] 2.6 Build the CLI-agent prompt as `Update the OpenSpec change
...
- [ ] 2.7 On `Completed`: `CommitAsync` with message `"updating specs for
...
- [ ] 2.8 Wrap the per-comment cycle (branch refresh through push) with
      `SpecRunnerOptions.TaskTimeout`; on timeout, call `StopAsync` on any
      in-flight CLI agent session.
- [ ] 2.9 On any thrown exception or timeout for a tracked PR: add the
...
- [ ] 2.10 Register `IUpdateWorkflowRunner` in `SpecRunner.Console`'s DI
...
- [ ] 3.1 Extend `PollingLoop.RunAsync` to accept `IUpdateWorkflowRunner`
...
- [ ] 3.2 Update `Program.cs` to resolve `IUpdateWorkflowRunner` and pass
...
- [ ] 4.2 Add unit tests for the already-reacted skip logic
      (`eyes`/`+1`/`confused` from the bot vs. a human reaction vs. no
      reaction).
- [ ] 4.3 Add unit tests for the untracked-PR path (explanatory reply,
      `confused` reaction, no git/CLI/state-store activity).
- [ ] 4.4 Add unit tests for the success path (branch fetch/switch/reset,
      ...
      state-store update).
- [ ] 4.5 Add unit tests for the error path (thrown exception mid-cycle)
      and the timeout path (session `StopAsync` called, `confused`
      reaction posted), including that processing continues to the next
      comment.
- [ ] 4.6 Add a unit test confirming two eligible comments in one scan
      pass are processed sequentially, not concurrently.
- [ ] 4.7 Add a unit test confirming `propose-workflow`,
...
- [ ] 4.8 Add/update the DI smoke test to confirm `IUpdateWorkflowRunner`
      resolves from the container.
- [ ] 4.9 Run `dotnet test SpecRunner/SpecRunner.sln` and confirm all
      tests pass.

## 5. Documentation

- [ ] 5.1 Update `SpecRunner/README.md` to describe the `/update` comment
...
- [ ] 6.1 Run `dotnet build SpecRunner/SpecRunner.sln` and confirm it
      succeeds with no errors.
- [ ] 6.2 Run `dotnet test SpecRunner/SpecRunner.sln` and confirm all
      tests pass.
