...
- [ ] 1.1 Replace `RepositoryOwner`/`RepositoryName` with a single
      `RepositoryUrl` string property on `SpecRunnerOptions`
      (`SpecRunner.Core`).
- [ ] 1.2 Update `SpecRunner.Console/appsettings.json`: remove
...
- [ ] 3.1 Add an HTTP-based `IRepositoryConnectionTester` implementation to
...
- [ ] 3.2 Map the HTTP response to a status: `200` → `Connected`; `401`/
      `403` → `AuthenticationFailed`; `404` → `RepositoryNotFound`; any
      transport-level exception (no HTTP response) → `NetworkError`.
- [ ] 3.3 Ensure the implementation never includes `GitHubToken` in the
      `Message` of any `RepositoryConnectionResult` it returns.
- [ ] 3.4 Register `IRepositoryConnectionTester` (and the `HttpClient` it
      needs, via `AddHttpClient`) in `SpecRunner.Console`'s DI container.

## 4. Serilog logging

- [ ] 4.1 Add `Serilog.Extensions.Hosting`, `Serilog.Sinks.Console`,
...
- [ ] 4.2 Wire `UseSerilog` into `Host.CreateApplicationBuilder` in
      `Program.cs`, reading sink/level configuration from the `Serilog`
      section via `ReadFrom.Configuration`.
- [ ] 4.3 Add a `Serilog` section to `appsettings.json` with a console sink
      and a rolling file sink configured for `fileSizeLimitBytes: 1048576`
      and `rollOnFileSizeLimit: true`.
- [ ] 4.4 Replace any remaining ad hoc console output in `Program.cs` with
